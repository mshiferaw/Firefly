...
function drawScene(pdraw = params.partsKeys)
{
	...
	for (var i=0; i<pdraw.length; i++){
		...
		var material = new THREE.ShaderMaterial( {
			...
			fragmentShader: myFragmentShader,function clearPartsMesh(pClear = params.partsKeys) {
function clearPartsMesh(pClear = params.partsKeys) {
	for (var i=0; i<pClear.length; i++){
		var p = pClear[i];
	...
	for (var i=0; i<pdraw.length; i++){
		...
		var material = new THREE.ShaderMaterial( {
			...
			uniforms: { //add uniform variable here
				...
				velType: {value: 0.},
				// ADDED HERE
				tex: {value: params.texture},
				useColorBar: {value: params.useColorBar}
			},
		...
		var temperature = new Float32Array( params.plotNmax[p]) 
		geo.addAttribute('Temperature', new THREE.BufferAttribute( temperature, 1))
		...
		for (var j=0; j<params.plotNmax[p]; j++){
			...
			if (includePoint){
				...
				if (params.parts[p].NormTemp != null){
					temperature[tindex] = params.parts[p].NormTemp[j];
					//console.log(temperature[tindex], params.parts[p].NormTemp[j])
					tindex++;
				}
...
function drawScene(pdraw = params.partsKeys)
{
	...
	for (var i=0; i<pdraw.length; i++){
		...
		if (params.toggle){
			var shader = myColorFragmentShader
		}
		else {
		...
		var metallicity = new Float32Array( params.plotNmax[p]) 
		geo.addAttribute('Metallicity', new THREE.BufferAttribute( metallicity, 1))
