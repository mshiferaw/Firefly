...
function defineParams(){
	ParamsInit = function() {
		...
		// gives place to texture in this object
		this.texture;
		// create boolean to toggle colormap
		this.toggle = false;
	};
...
function applyOptions(){
	...
	if (params.parts.options.hasOwnProperty('center')){
		...
		params.parts.options.center = [params.center.x, params.center.y, params.center.z];//all "global" variables are contained within params object
var params;
function defineParams(){
	ParamsInit = function() {
...
function calcNormTemp(p){
	...
	var tdif = max - min;
	//console.log('Norm Temp', params.parts[p].NormTemp)
	for (var i=0; i<params.parts[p].log10Temperature.length; i++){
